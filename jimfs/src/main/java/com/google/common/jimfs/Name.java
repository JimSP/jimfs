...
/**
 * Immutable representation of a file name. Used both for the name components of
 * paths and as the keys for directory entries.
 *
 * <p>
 * A name has both a display string (used in the {@code toString()} form of a
 * {@code Path} as well as for {@code Path} equality and sort ordering) and a
 * canonical string, which is used for determining equality of the name during
 * file lookup.
 *
 * <p>
 * Note: all factory methods return a constant name instance when given the
 * original string "." or "..", ensuring that those names can be accessed
 * statically elsewhere in the code while still being equal to any names created
 * for those values, regardless of normalization settings.
 *
 * @author Colin Decker
 */
final class Name implements Serializable, Comparable<Name> {

    private static final long serialVersionUID = -7179924219359377042L;
    ...
    /**
     * Creates a name with the given display representation and the given canonical
     * representation.
     */
    ...
    public int compareTo(Name name) {
        return (display + canonical).compareTo(name.display + name.canonical);
    }
